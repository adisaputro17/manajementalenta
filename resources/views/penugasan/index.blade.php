    ...
    <td>
        @if(auth()->user()->role == 'ADMIN')
            @if($d->row_status == 'PENDING')
                <form action="{{ route('penugasan.approve',$d->id) }}" method="POST" class="d-inline">
                    @csrf
            ...
            <a href="{{ route('penugasan.edit',$d->id) }}" class="btn btn-warning btn-sm">Edit</a>

            <form action="{{ route('penugasan.destroy',$d->id) }}" method="POST" class="d-inline">
                @csrf
                @method('DELETE')
...
<div class="modal fade" id="rejectModal{{ $d->id }}" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('penugasan.reject',$d->id) }}" method="POST">
            @csrf
