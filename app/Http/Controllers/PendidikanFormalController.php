...
class PendidikanFormalController extends Controller
{
    public function index()
    {
        if(auth()->user()->role == Role::ADMIN){
            $data = PendidikanFormal::with('pegawai')->latest()->paginate(10);
        }else{
            $data = PendidikanFormal::where('pegawai_id', auth()->id())->latest()->paginate(10);
        }
    ...
    public function edit(PendidikanFormal $pendidikan)
    {
        abort_if(
            auth()->user()->role!=Role::ADMIN && $pendidikan->pegawai_id!=auth()->id(),
            403
        );
    ...
    public function update(Request $request, PendidikanFormal $pendidikan)
    {
        $request->validate([
            'pegawai_id'=>'required',
            'tingkatan'=>'required'
    ...
    public function destroy(PendidikanFormal $pendidikan)
    {
        if($pendidikan->bukti){
            Storage::delete($pendidikan->bukti);
        }
